...
<template lang="html">
  <div class="box">
    <div  v-show="!isTesting">
      ...
      <p v-if="!testData || testData.length === 0" class="empty-message-prompt">暂无练习</p>
      <table v-else class="table">
        ...
        <tbody>
          <tr v-for="(item,index) in testData">
            ...
            <td>{{ item.score }}</td>
            <td>
              <button @click="startTest(index)" class="is-small button" type="button" name="button">开始练习</button>
...
export default {
  data() {
    return {
      ...
      data: null,
      //
      isTesting: false,  // 是否已经开始练习
      paperId: null,  // 试卷ID
      examId: null, // 练习ID testID
      examTime: null,
      selectedTestData: null,  // 当前选中的练习
